...
import { VideoService } from '../services/VideoService'; // <-- IMPORT DU SERVICE DE RECHERCHE
...
export default function RegieDirector() {
    ...
    const [inputValue, setInputValue] = useState(''); // Remplace inputUrl pour servir aussi à la recherche
    ...
    // 1. Récupération de la playlist sauvegardée (UNIQUE À CETTE ROOM)
    useEffect(() => {
        if (!roomId) return;
        const storageKey = `regie_director_playlist_${roomId}`; // <-- Clé unique par régie
        try {
    ...
    // 2. Sauvegarde automatique de la playlist (UNIQUE À CETTE ROOM)
    useEffect(() => {
        if (!roomId) return;
        const storageKey = `regie_director_playlist_${roomId}`; // <-- Clé unique par régie
        try { localStorage.setItem(storageKey, JSON.stringify(playlist)); } catch (e) {}
    }, [playlist, roomId]);
    ...
    useEffect(() => {
        const checkPermission = async () => {
            ...
            if (!isManager) navigate(`/regie/${roomId}/viewer`);
        };
        checkPermission();
    }, [roomId, navigate]);
    ...
    // --- NOUVELLE LOGIQUE INTELLIGENTE D'AJOUT ET RECHERCHE ---
    const handleAddOrSearch = async () => {
        const val = inputValue.trim();
        if (!val) return;

        // 1. Est-ce que c'est une URL directe ?
        ...
        // 2. Ce n'est pas une URL, on lance la recherche !
    ...
    const handleAddSearchResult = (youtubeId) => {
        if (playlist.length < 10 && !playlist.includes(youtubeId)) {
            ...
            // Si la playlist est pleine, on ferme automatiquement les résultats
    ...
    const pushRegieState = useCallback(async (videoId, overrides = {}) => {
        ...
        const payload = {
            room_id: Number(roomId), video_id: videoId,
            video_cursor: overrides.video_cursor ?? progressRefs.current[videoId] ?? 0,
            is_playing: true, updated_at: new Date().toISOString()
        };
        try { await supabase.from('regie_state').upsert(payload, { onConflict: 'room_id' }); } catch (e) {}
    }, [roomId]);
    ...
    const handlePlay = (videoId) => { playingRefs.current[videoId] = true; setLocalPlaying(prev => ({ ...prev, [videoId]: true })); };
    const handlePause = (videoId) => { playingRefs.current[videoId] = false; setLocalPlaying(prev => ({ ...prev, [videoId]: false })); };
    const handleProgress = (videoId, seconds) => { progressRefs.current[videoId] = seconds; };


    // COMPOSANT RÉUTILISABLE POUR AFFICHER LES RÉSULTATS
    const renderSearchResults = () => {
        ...
        return (
            <Box sx={{ 
                ...
                <Box sx={{ p: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                    <Typography variant="caption" sx={{ color: 'text.secondary', ml: 1 }}>Résultats YouTube</Typography>
                    <IconButton size="small" onClick={closeSearch}><CloseIcon fontSize="small"/></IconButton>
                </Box>
                        ...
                        return (
                            <ListItem key={youtubeId} divider sx={{ alignItems: 'flex-start', py: 1 }}>
                                <Stack direction="row" spacing={1.5} sx={{ width: '100%', alignItems: 'center' }}>
                                    <Box component="img" src={thumbnail} sx={{ width: 80, height: 45, borderRadius: 1, objectFit: 'cover' }} />
                                    <Typography variant="body2" sx={{ flexGrow: 1, fontWeight: 600, lineHeight: 1.2 }}>{title}</Typography>
                                    <IconButton size="small" onClick={() => handleAddSearchResult(youtubeId)} disabled={playlist.length >= 10 || isAlreadyAdded} color={isAlreadyAdded ? "success" : "default"}>
                ...
                <Box sx={{ maxWidth: 600, mx: 'auto', mt: 4 }}>
                    ...
                    <Box sx={{ mb: 4 }}>
                        <Stack direction="row" spacing={2}>
                            <TextField fullWidth label="Rechercher (ex: lofi) ou coller une URL..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} disabled={playlist.length >= 10} onKeyDown={(e) => e.key === 'Enter' && handleAddOrSearch()} />
                            <Button variant="contained" onClick={handleAddOrSearch} disabled={playlist.length >= 10 || isSearching || !inputValue}>
                    ...
                    <Stack spacing={1} sx={{ mb: 4 }}>
                        {playlist.map((id, i) => (
                            <Box key={i} sx={{ p: 2, bgcolor: 'background.paper', borderRadius: 1, display: 'flex', alignItems: 'center', gap: 2 }}>
                                ...
                                <IconButton size="small" onClick={() => handleRemoveVideo(id)} color="error"><CloseIcon /></IconButton>
                            </Box>
                        ))}
                        {playlist.length === 0 && <Typography variant="body2" color="text.secondary">Ajoutez des liens YouTube pour préparer vos écrans.</Typography>}
                    </Stack>

                    <Button variant="contained" color="secondary" fullWidth size="large" disabled={playlist.length === 0} onClick={() => setPhase('live')}>Commencer la régie</Button>
                </Box>
            </Section>
        );
    }
        ...
        <Box sx={{ p: 2, height: 'calc(100vh - 64px)', bgcolor: '#121212', overflowY: 'auto' }}>
            ...
            <Box sx={{ mb: 3, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', md: 'center' }, gap: 2 }}>
                <Typography variant="h4" sx={{ color: 'white', display: 'flex', alignItems: 'center', gap: 2 }}>
                    Live - Régisseur
                    {liveVideoId && <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, bgcolor: '#ff0000', px: 1.5, py: 0.5, borderRadius: 4, fontSize: '1rem', fontWeight: 'bold' }}><SensorsIcon fontSize="small"/> EN DIRECT</Box>}
                </Typography>

                {/* BARRE DE RECHERCHE FLOTTANTE */}
                <Box sx={{ position: 'relative', width: { xs: '100%', md: '450px' }, zIndex: 10 }}>
                    <Stack direction="row" spacing={1}>
                        <TextField size="small" placeholder="Rechercher sur YouTube ou URL..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} disabled={playlist.length >= 10} onKeyDown={(e) => e.key === 'Enter' && handleAddOrSearch()} sx={{ bgcolor: 'rgba(255,255,255,0.1)', input: { color: 'white' }, borderRadius: 1, flex: 1 }} />
                        <Button variant="contained" color="primary" onClick={handleAddOrSearch} disabled={playlist.length >= 10 || !inputValue || isSearching} sx={{ minWidth: '120px' }}>
                            {isSearching ? <CircularProgress size={24} color="inherit" /> : `Rechercher/Ajouter (${playlist.length}/10)`}
                        </Button>
                    </Stack>
                    {renderSearchResults()}
                </Box>
                ...
                {playlist.map((videoId) => {
                    ...
                    return (
                        <Grid item xs={12} sm={6} md={4} key={videoId}>
                            <Box sx={{ bgcolor: 'black', borderRadius: 2, overflow: 'hidden', p: 1, position: 'relative', border: isLive ? '4px solid #ff0000' : '2px solid transparent', boxShadow: isLive ? '0 0 20px rgba(255, 0, 0, 0.7)' : '0 0 10px rgba(0,0,0,0.5)', transform: isLive ? 'scale(1.02)' : 'scale(1)', transition: 'all 0.3s ease-in-out' }}>
                                
                                <IconButton size="small" onClick={() => handleRemoveVideo(videoId)} sx={{ position: 'absolute', top: 12, right: 12, zIndex: 10, bgcolor: 'rgba(0,0,0,0.6)', color: 'white', '&:hover': { bgcolor: 'error.main' } }}>
                                ...
                                <Button fullWidth variant={isLive ? "outlined" : "contained"} color="error" startIcon={isLive ? <SensorsIcon /> : <SendIcon />} onClick={() => handleBroadcast(videoId)} sx={{ mt: 1, fontWeight: 'bold', bgcolor: isLive ? 'rgba(255, 0, 0, 0.1)' : 'error.main' }}>
