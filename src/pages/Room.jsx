...
const getRoleBadge = (role) => {
    switch (role) {
        case ROLES.OWNER:
            return <Chip label="Owner" size="small" color="primary" sx={{ ml: 1 }} />;
        case ROLES.MANAGER:
            return <Chip label="Manager" size="small" color="secondary" sx={{ ml: 1 }} />;
        default:
            return <Chip label="Member" size="small" sx={{ ml: 1 }} />;
    }
};
...
export default function Room() {
    ...
    const canInvite = userRole === ROLES.OWNER || userRole === ROLES.MANAGER;
        ...
        const channel = RealtimeService.joinBroadcast(roomId, (event) => {
            ...
            if (event.type === 'RESPONSE_MANAGER' && event.targetId === user.id) {
                ...
                if (event.approved) {
                    setSnackbar({
                        open: true,
                        message: t('room.request_accepted'),
                        severity: 'success',
                    });
                } else {
                    setSnackbar({
                        open: true,
                        message: t('room.request_rejected'),
                        severity: 'error',
                    });
                }
            }
    ...
    const handleRequestManager = async () => {
        ...
        await broadcastChannel.send('REQUEST_MANAGER', {
            userId: user.id,
    ...
    const handleApproveRequest = async () => {
        ...
        try {
            ...
            broadcastChannel.send('RESPONSE_MANAGER', {
                targetId: pendingRequestUser.id,
                approved: true,
            });
            setSnackbar({
                open: true,
            ...
            try {
                ...
                if (room?.ownerId === user.id) role = 'owner';
                    ...
                    if (currentUserMember) {
                        role = currentUserMember.isOwner
                            ? 'owner'
                            : currentUserMember.is_manager
                              ? 'manager'
                              : 'member';
                    }
                }
    ...
    useEffect(() => {
        ...
        if (room.ownerId === user.id || userRole === 'owner' || userRole === 'manager') {
            setIsModerator(true);
            return;
        }
    ...
    const submitPin = async () => {
        try {
            ...
            if (!ok) {
                setPinError(t('room.pin_invalid'));
                return;
            }
    ...
    const handleVideoSelect = async (url, title = null) => {
        ...
        if (videoId) {
            requirePin(() => canControlVideo && changeVideo(`https://www.youtube.com/watch?v=${videoId}`, title));
        }
    };
        ...
        const handleRoleChange = (payload) => {
            ...
            if ((payload.new?.user_id || payload.old?.user_id) === user.id) {
                if (payload.eventType === 'DELETE') {
                    setIsKicked(true);
                } else if (payload.new?.is_manager) {
                    setSnackbar({
                        open: true,
                        message: t('room.promoted_message'),
                        severity: 'success',
                    });
                }
            }
    ...
    const loadBannedUsers = async () => {
        if (roomId) {
            try {
                setBannedUsers(await BanRepository.listBannedUsers(roomId));
            } catch (e) {}
        }
    };

    useEffect(() => {
        if (activeTab === 'moderation' && modView === 'banned') loadBannedUsers();
    }, [activeTab, modView]);

    const getMemberRole = (m) =>
        m ? (m.isOwner ? ROLES.OWNER : m.is_manager ? ROLES.MANAGER : ROLES.MEMBER) : null;

    const canInteract = (targetRole, targetId) =>
        userRole &&
        targetId !== user?.id &&
        (userRole === ROLES.OWNER || (userRole === ROLES.MANAGER && targetRole === ROLES.MEMBER));
    ...
    const handleAction = async (action, target) => {
        ...
        try {
            if (action === 'promote') await RoleService.promote(roomId, target.userId);
                ...
                await BanRepository.banUser(roomId, target.userId);
    ...
    const handleUnbanUser = async (id, name) => {
        if (window.confirm(t('confirm.unban_user', { user: name }))) {
            await BanRepository.unbanUser(roomId, id);
            loadBannedUsers();
        }
    };
    ...
    if (!room) {
        return (
            <Section>
                <Typography>{t('room.not_found')}</Typography>
            </Section>
        );
    }
    ...
    if (isBanned) {
        return (
            <Section>
                <Typography color="error">{t('room.banned_message')}</Typography>
            </Section>
        );
    }
    ...
    if (isKicked) {
        return (
            <Box component={Section} sx={{ p: 4, textAlign: 'center', mt: 4 }}>
                ...
                >
                    <WarningIcon fontSize="large" /> {t('room.kicked_title')}
                </Typography>
        ...
        <Section>
                            ...
                            {canControlVideo && playlistItems.length > 1 && (
                                <Box sx={{ display: 'flex', gap: 1, mt: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
                                    <Button
                                        ...
                                        onClick={() =>
                                            requirePin(() => {
                                                const v = syncVideoId || currentVideoId;
                                                const p = getPrevVideo(v);
                                                if (p) changeVideo(p.url, p.title);
                                            })
                                        }
                                    ...
                                    <Button
                                        ...
                                        onClick={() =>
                                            requirePin(() => {
                                                const v = syncVideoId || currentVideoId;
                                                const n = getNextVideo(v);
                                                if (n) changeVideo(n.url, n.title);
                                            })
                                        }
                                        disabled={!currentVideoId && !syncVideoId}
                                    >
                        ...
                        >
                            <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                                ...
                                >
                                    ...
                                    {isModerator && (
                                        <Tab label={t('room.moderation')} value="moderation" sx={{ minWidth: '60px' }} />
                                    )}
                                </Tabs>
                            </Box>
                            ...
                            <Box sx={{ flex: 1, overflowY: activeTab === 'chat' ? 'hidden' : 'auto', p: 1 }}>
                                ...
                                {activeTab === 'chat' && (
                                    <Box sx={{ height: '100%', minHeight: 300 }}>
                                        <ChatBox roomId={roomId} isBanned={isBanned} isModerator={isModerator} />
                                    </Box>
                                )}
                                                ...
                                                {allMembers
                                                    ...
                                                    .map((m) => (
                                                        <ListItem
                                                            key={m.userId}
                                                            secondaryAction={
                                                                canInteract(getMemberRole(m), m.userId) && (
                                                                    <IconButton
                                                                        edge="end"
                                                                        onClick={(e) => {
                                                        ...
                                                        >
                                                            ...
                                                            <ListItemText
                                                                ...
                                                                secondary={getRoleBadge(getMemberRole(m))}
                                                            />
                                                        </ListItem>
                                                    ))}
                                            </List>
                                            ...
                                            <List dense>
                                                {bannedUsers.map((b) => (
                                                    <ListItem
                                                        ...
                                                        secondaryAction={
                                                            <Button
                                                                size="small"
                                                                color="success"
                                                                onClick={() => handleUnbanUser(b.userId, b.name)}
                                                            >
                                                                {t('action.unban')}
                                                            </Button>
                                                        }
                                                    >
                                                        ...
                                                        <ListItemText
                                                            primary={b.name}
                                                            secondary={new Date(b.bannedAt).toLocaleDateString()}
                                                        />
                                                    </ListItem>
                                                ))}
                                            </List>
            ...
            >
                <Alert
                    ...
                    action={
                        <Box>
                            <Button color="inherit" size="small" onClick={handleRejectRequest}>
                                REFUSER
                            </Button>
                            <Button
                                color="inherit"
                ...
                >
                    {pendingRequestUser?.name} demande à devenir Manager
                </Alert>
            </Snackbar>
            ...
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
                {selectedMember && (
                    <Box>
                        ...
                        <MenuItem
                            onClick={() => handleAction('ban', selectedMember)}
                            sx={{ color: 'error.main' }}
                        >
                            {t('action.ban')}
                        </MenuItem>
            ...
            <Dialog open={pinOpen} onClose={closePinDialog} maxWidth="xs" fullWidth>
                <DialogTitle>
                    {pinMode === 'set' ? t('room.pin_set_title') : t('room.pin_enter_title')}
                </DialogTitle>
                <DialogContent>
                    <TextField
                        autoFocus
                        ...
                        error={!!pinError}
                        helperText={pinError}
                        onKeyDown={(e) => e.key === 'Enter' && (pinMode === 'set' ? handleSavePin() : submitPin())}
                    />
                </DialogContent>
                <DialogActions>
                    <Button onClick={closePinDialog}>{t('common.cancel')}</Button>
                    {pinMode === 'set' && (
                        <Button color="error" onClick={handleDisablePin}>
                            {t('room.pin_disable')}
                        </Button>
                    )}
                    <Button
